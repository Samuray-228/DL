...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        train_ind, val_ind = torch.utils.data.random_split(texts, [1 - self.VAL_RATIO, self.VAL_RATIO], 
                                                           torch.Generator().manual_seed(self.TRAIN_VAL_RANDOM_SEED))
        train_texts, val_texts = list(map(texts.__getitem__, train_ind.indices)), list(map(texts.__getitem__, val_ind.indices))
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        indices[1:length+1] = torch.LongTensor(self.indices[item][:length])
