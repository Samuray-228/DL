...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.var = np.sum(self.input_mean**2, axis=0) / input.shape[0]
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        :return: array of shape (batch_size, num_features)
        """
        grad_input = self.weight * grad_output if self.affine else grad_output
        if self.training:
            return self.inv_sqrt_var * (grad_output.shape[0] * grad_input - np.sum(grad_input, axis=0) - self.norm_input * np.sum(grad_input * self.norm_input, axis=0)) / grad_output.shape[0]
        return self.inv_sqrt_var * grad_input
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.modules) - 1, 0, -1):
            x = self.modules[i].compute_grad_input(self.modules[i - 1].output, grad_input)
            self.modules[i].update_grad_parameters(self.modules[i - 1].output, grad_input)
            grad_input = x
        x = self.modules[0].compute_grad_input(input, grad_input)
